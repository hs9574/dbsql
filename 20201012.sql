...
4. 관계 정의
    ...
    1. 식별속성을 먼저 고려 해볼 것
    2. 키가 복잡해진다, 손자 엔터티가 나올거 같지 않음
       ==> 비식별 관계
       (학생 자식 엔터티가 수강, 손자라는 것은 수강에서 파생된 엔터티)
...
Super Type, Sub Type을 물리 모델로 변경하게 될 경우 선택방안은 3가지가 존재
1. super type + 모든 sub Type 통합 (생성되는 테이블 수 : 1)
   하나의 테이블로 모든 속성을 넣는 형태로, 단점은 null값을 갖는 컬럼이 많아 질 수 있다
...
정규화 순서 : 1 정규형 - 2 정규형 - 3 정규형 - BCNF 정규형 - 4 정규형 - 5 정규형
    ...
    실무에서는 1~3정규형 까지만 진행, WHY? 3정규형 이후 부터는 발생 빈도가 낮아지고, 수고가 커짐

함수 종속
a라는 값을 통해 b라는 값이 정해지는 것 (a -> b)
사번 -> 사원이름, 담당업무, 소속부서, 급여, 매니져
7369 -> brown
brown -> 사번 유추 가능?

1 정규형 : 반복속성을 별도의 엔터티로 분리
          (게시글의 첨부파일1~5번까지 관리를 위한 5개의 컬럼을 첨부파일 엔터티라는 별도의 엔터티로 분리
           ==> 첨부파일이 추가가 될때 컬럼 구조 변경이 아니라 데이터의 행추가로 문제점을 해결
               (테이블 구조 변화가 없다)
